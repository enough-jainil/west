...
#         app3.app2()  # Change this line from app3.app3() to app3.app2()
...
#         quality = app3.quality  # Use app3.quality instead of app2.quality
#         counts = [quality.count(0), quality.count(1)]
#         if len(quality) == 0:
#             st.error("Please run some inference on water potability test")
#         else:
#             ans = max(set(quality), key=quality.count)
...
#             plt.title('Proportions Of Water Potability')
#             st.pyplot()
...
def main():
    ...
    if selected_model == 'Home':
        ...
        st.header('Water Quality for Potability')
        quality = app3.quality
        counts = [quality.count(0), quality.count(1)]
        if len(quality) == 0:
            st.error("Please run some inference on water potability test")
        else:
            # Convert to integer and ensure it's either 0 or 1
            ans = int(round(np.mean(quality)))
            ans = max(0, min(ans, 1))  # Ensure ans is either 0 or 1
            labels_wqa = ['Fit for use', 'Polluted']
            qwa = labels_wqa[ans]
            colors = ['#1f77b4', '#ff7f0e']
            sns.set_style("whitegrid")
